...
Core analysis owns summary + category labels (`policy_areas` / `policy_implications.categories`) and **hidden provisions** (`hidden_provisions` → `HiddenProvision` table; UI reads the table only). **Stakeholder Analysis** and deep **Policy Analysis** are separate async Gemini enrichers (`services/analysis_enrichers.py`); they must not block `display_ready`. Bill detail UI: **Policy Areas** (badges) ≠ **Policy Analysis** (narrative) ≠ **N hidden provisions detected** (collapsible, DB-backed).
...
4. **Enrichments** — after core completes, **routes, RSS, and backfill** queue enrichments via `services/enrichment_queue.queue_downstream_enrichments` → `run_downstream_enrichments` under a separate `KIND_ENRICH` lease. Ops: `enrichment_queued` / `enrichment_finished` with `source`/`pipeline` of `routes` | `rss` | `backfill`. Quota checks must use `enrichment_quota_ok()` → `get_rate_limit_status()["remaining_requests"]` (or `get_quota_info()["current_usage"]`). **Never** read `get_quota_info()["status"]["safe_remaining_requests"]` — that key is not under `status` and falsely reports 0. On real RPM deferral: keep enrichments `pending`, do not churn analysis versions, cooldown until minute reset. Offline pipelines also re-queue when revisiting a bill that still has pending stubs.
...
6. **Free tier** — Google AI Studio typically ~15–30 RPM and ~1,500 RPD (daily reset midnight Pacific). Local limiter uses 15 RPM via **shared** `GeminiRateBudget` / `get_shared_ai_analyzer()` (not per-ingestor instances). Over limit → API 429 `RESOURCE_EXHAUSTED`. Free-tier prompts may be used to improve Google products. Concurrent search/RSS/backfill also coordinate via `bill_work_lease` so the same bill is not double-analyzed.
...
| **ETL** | Ingestion, Congress API, RSS, bill processors, backfill | `services/congress_*`, `services/rss_*`, `services/*bill_processor*`, `services/bill_sync.py`, `services/backfill_*`, `services/bill_work_lease.py` |
| **Database** | Models, migrations, indexes, versioning | `db_models.py`, `migrations/`, `manage.py` |
| **Analysis** | Gemini analysis, chunking, categories, enrichers, hidden provisions | `services/enhanced_ai_analyzer.py`, `services/gemini_rate_budget.py`, `services/analysis_enrichers.py`, `services/analysis_*`, `utils/bill_chunker.py`, `utils/text_processing.py` |
...
| `legislai-etl` | Ingestion / Congress / RSS / backfill / bill_sync |
