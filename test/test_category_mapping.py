#!/usr/bin/env python3
"""
Test script to verify bill_category_mapping population
"""
...
def test_category_mapping():
    """Test the complete workflow with a bill that has full text"""
    try:
        with app.app_context():
            logger.info("Testing Bill Category Mapping Population")
            logger.info("=" * 50)
            ...
            # Test with HR2670 from 118th Congress (known to have text)
            congress = 118
            bill_type = 'hr'
            bill_number = 2670
            
            logger.info(f"Testing with bill: {congress}-{bill_type.upper()}{bill_number}")
            ...
            if existing_bill:
                ...
                if not bill_data:
                    logger.error("Could not fetch bill data from Congress API")
                    return False
            ...
            if not full_text:
                logger.error("Could not fetch full text from Congress API")
                return False
            
            logger.info(f"✅ Successfully fetched full text ({len(full_text):,} characters)")
...
def main():
    """Main test function"""
    logger.info("Starting Bill Category Mapping Test")
    logger.info("=" * 60)
