...
LegislAI helps people understand U.S. legislation. It pulls bills from Congress, stores them, runs AI analysis, and presents clear summaries, policy themes, risk signals, and optional alerts in a web app.

The goal is straightforward: turn dense bill text into something a researcher, journalist, advocate, or curious constituent can actually use—without waiting for a news cycle to decode it.

It runs on free-tier Gemini by default, and you can tune models, rate limits, and analysis heuristics when you build something larger on top of it.

## What it does

1. **Ingests** bills from several entry points (not RSS alone).
...
| **Historical backfill** | Careful catch-up over past congresses or gaps in your database |
...
You will want `GEMINI_API_KEY` and `CONGRESS_API_KEY`. Admin/ops sign-in uses `LEGISLAI_ADMIN_*` from the template.
