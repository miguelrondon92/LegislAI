...
if echo "$prompt" | grep -Eiq '(show|print|dump|cat|read).{0,40}(\.env|private key|api key|secret key|ADMIN_PASSWORD|MAIL_PASSWORD)|(what is|what.s) (my|the) (api key|secret|password)'; then
  jq -n '{
    permission: "deny",
